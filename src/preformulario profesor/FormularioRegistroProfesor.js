...
const Modal = ({ isOpen, onClose, onSubmit, formData, handleChange, isEditMode, viewMode }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="relative w-full max-w-lg p-8 bg-black bg-white rounded-lg shadow-md  overflow-y-auto max-h-[80vh]">
        <button
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          {Object.keys(formData).map((key) => (
            <input
              key={key}
              type={key === 'añosexperiencia' ? 'number' : 'text'}
              name={key}
          ...
          <div className="flex justify-end">
            <button
              type="button"
              onClick={onClose}
              className="text-white  bg-blue-800 hover:bg-yellow-400 p-2 rounded transition duration-300 transform hover:scale-110 mr-2"
            >
              Cancelar
            </button>
            {!viewMode && (
...
const Table = ({ data, handleEdit, handleViewDetails, handleDelete }) => (
  <table className="min-w-full bg-white rounded-md shadow-md">
    <thead>
      <tr>
        {['Nombres', 'Apellido Paterno', 'Apellido Materno', 'Acciones'].map((header) => (
    ...
    <tbody>
      {data.map((item) => (
        <tr key={item.id}>
          {['nombres', 'apellidopaterno', 'apellidomaterno'].map((field) => (
...
const FormularioProfesor = () => {
  ...
  const fetchData = async () => {
    try {
      const result = await axios.get('http://localhost:5000/api/profesor');
      setData(result.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/profesor', formData);
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/api/profesor/${formData.id}`, formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/profesor/${id}`);
